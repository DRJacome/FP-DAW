...
function resolver_3_15(numero) {
	...
	if (isNaN(numero) || numero<=1 || (numero % 1 != 0)) {
		...
		if (ePrimo(numero)) { texto += 'O número é primo<br>'; }
...
function resolver_3_22() {
	xanela = true;
	texto = 'Ata que número quere calcular?';	
	while (xanela) {
		var resposta = prompt(texto);
		if (isNaN(resposta) || resposta<2 || (resposta % 1 != 0)) {
			texto = 'Menos cachondeos e mete un número válido!!!\n' + texto;
		} else {
			xanela = false;
		}	
	}
...
function resolver_3_23(inicial,final){
	var saida = '';
	if (!noNatural(inicial) && !noNatural(final)) {
		inicial = parseInt(inicial);
		final = parseInt(final);
		if (final >= inicial) {
			for (var i=inicial; i<=final; i++) {
				if (!buscarFicheiro(completar(i))) { saida += i + ' '; }
			}
		} else {
			saida = 'ERRO: o número inicial non pode ser maior que o final';
		}	
	} else {
		saida = 'ERRO: os números inicial e final deben ser maiores que 0';
	}
	if (saida == '') { saida = 'Non faltan ficheiros'; }
	amosarResultado(saida);	
}

function noNatural(numero) { // numeroEnteiroPositivoDistintoDeCero
	return (isNaN(numero) || numero<=0 || (numero % 1 != 0));
}
...
function completar(numero) {
	const DIXITOS = 3;
	numero = '00' + numero;
	return numero.substr(-3);
}

function buscarFicheiro(numero) {
	for (var i=0; i<document.getElementById('cartafol').files.length; i++) {
		var ficheiro = document.getElementById('cartafol').files[i].name;
		if (numero == ficheiro.substr(0,3)) { return true; }
	}
	return false;	
}